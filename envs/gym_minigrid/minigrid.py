...
class Goal(WorldObj):
    def __init__(self, obj_type='goal', color='green'):
...
class SubGoal(Goal):
    def __init__(self):
        super().__init__(obj_type='subgoal', color='cyan')
    ...
    def decode(array):
        ...
        for i in range(width):
            for j in range(height):
                ...
                # We only use: wall, floor, goal, lava. And only wall has color grey (check)
                # is_open/is_locked doesn't matter for us - only for doors
                # So, set default color as 5, and default state as 0, and we only require 1st channel as state information
                if objType == 'wall':
                    v = Wall(color)
                elif objType == 'floor':
                    v = Floor(color)
...
class MiniGridEnv(gym.Env):
    ...
    def get_obs_render(self, obs, tile_pixels=CELL_PIXELS//2):
        ...
        if self.obs_render == None:
            from envs.gym_minigrid.rendering import Renderer
